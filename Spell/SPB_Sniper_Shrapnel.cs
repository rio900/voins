...
{
    public class SPB_Sniper_Shrapnel : ISpell
    {
        /// <summary>
        ...
        void _firstTimer_Completed(object sender, object e)
        {
            ...
            if (Duration-- >= 0)
            {
                ...
                for (int i = 0; i < _oldCall.IUnits.Count; i++)
                {
                    ...
                    {
                        item.GatDamage(0, _bullet.DemageMagic , 0, _unit);

                        Buff buff = item.Buffs.FirstOrDefault(p => p.Name == "SniperShrapnel");
                        if (buff != null)
                        {
                            buff.Duration = 2;
                        }
